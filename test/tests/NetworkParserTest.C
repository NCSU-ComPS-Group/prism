...
TEST_F(NetworkParserTest, LumpedSpecies)
{
  ...
  EXPECT_EQ(s_list.size(), uint(3));
  EXPECT_EQ(r_list.size(), uint(5));
  ...
  for (const auto & r : r_list)
  {
    ...
    {
      case 0:
        EXPECT_EQ(r_data.size(), uint(2));
        EXPECT_EQ(p_data.size(), uint(2));
        for (const auto & s : r_data)
        {
          if (np.getSpeciesNameById(s.id) == "N2*")
          {
            EXPECT_EQ(s.id, SpeciesId(0));
            EXPECT_EQ(s.occurances, uint(1));
      ...
      case 3:
        ...
        break;
      case 4:
        EXPECT_EQ(r_data.size(), uint(1));
        EXPECT_EQ(p_data.size(), uint(1));
        for (const auto & s : r_data)
        {
          if (np.getSpeciesNameById(s.id) == "N2*")
          {
            EXPECT_EQ(s.id, SpeciesId(0));
            EXPECT_EQ(s.occurances, uint(2));
            EXPECT_EQ(s.stoic_coeff, 0);
          }
        }
        for (const auto & s : p_data)
